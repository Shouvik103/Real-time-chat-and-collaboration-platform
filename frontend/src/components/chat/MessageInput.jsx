...
export function MessageInput({ editingMessage, onCancelEdit }) {
  ...
  return <div className="border-t border-chat-border bg-chat p-3 relative flex flex-col gap-2">{
    /* Editing header */
  }{editingMessage && <div className="flex items-center justify-between bg-brand/10 text-brand-light px-3 py-1.5 rounded-md text-xs font-medium border border-brand/20"><span>Editing message</span><button onClick={onCancelEdit} className="text-brand-light hover:text-white transition-colors p-0.5 rounded-full hover:bg-brand/20"><XMarkIcon className="h-4 w-4" /></button></div>}{
    /* Emoji Picker */
