...
export function ChatArea() {
  ...
  const storeMessages = useChatStore(
    (s) => activeChannelId ? s.messages[activeChannelId] ?? [] : []
  );
  ...
  if (!activeChannelId) {
    return <div className="flex flex-1 items-center justify-center bg-chat"><div className="text-center space-y-5 px-8 max-w-sm"><div className="mx-auto h-24 w-24 rounded-full bg-chat-surface border border-chat-border flex items-center justify-center"><svg
      viewBox="0 0 120 120"
      className="h-20 w-20"
  ...
  >{isFetchingNextPage && <div className="flex justify-center py-3"><Spinner className="h-5 w-5 text-brand" /></div>}{isLoading ? <div className="flex justify-center py-10"><Spinner className="h-6 w-6 text-brand" /></div> : storeMessages.length === 0 ? <div className="flex flex-col items-center justify-center py-20 gap-2"><ChatBubbleLeftRightIcon className="h-8 w-8 text-slate-600" /><p className="text-sm text-slate-500">
                No messages yet. Start the conversation!
              </p></div> : storeMessages.map((msg, i) => {
    const prev = storeMessages[i - 1];
    const showAvatar = !prev || prev.senderId !== msg.senderId || !isSameDay(prev.createdAt, msg.createdAt);
    const showDivider = !prev || !isSameDay(prev.createdAt, msg.createdAt);
    const isCurrentUser = currentUser?.id === msg.senderId;
    const currentAvatarUrl = isCurrentUser ? currentUser?.avatarUrl : activeWorkspace?.members?.find((m) => m.id === msg.senderId)?.avatarUrl;
    return <div key={msg.id}>{showDivider && <div className="flex items-center gap-3 px-4 my-3"><div className="flex-1 border-t border-chat-border" /><span className="text-xs font-medium text-slate-500">{formatDateDivider(msg.createdAt)}</span><div className="flex-1 border-t border-chat-border" /></div>}<MessageBubble
      message={msg}
