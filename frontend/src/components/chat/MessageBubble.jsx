...
export function MessageBubble({
  ...
  if (message.deleted) {
    return <div className="px-4 py-1"><p className="text-sm italic text-slate-600">This message was deleted</p></div>;
  }
  return <div
    className={clsx(
  ...
  >{
    /* Avatar */
  }<div className="w-9 shrink-0 self-end">{showAvatar && <Avatar
    name={message.senderName}
    src={currentAvatarUrl !== void 0 ? currentAvatarUrl : message.senderAvatar}
    size="md"
  />}</div>{
    /* Bubble + meta */
  }<div
  ...
  ><button
    ...
    title="Edit"
  ><PencilSquareIcon className="h-3.5 w-3.5" /></button><button
  ...
  ><time className="text-[10px] text-slate-500 whitespace-nowrap">{formatMessageTime(message.createdAt)}</time></div></div>{
    /* Reactions */
  }{message.reactions.length > 0 && <div className="mt-1 flex flex-wrap gap-1">{aggregateReactions(message.reactions).map(({ emoji, count, hasOwn }) => <button
    key={emoji}
    onClick={() => onReact?.(message.id, emoji)}
    className={clsx(
      "inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs transition-colors",
      hasOwn ? "border-brand/40 bg-brand/10 text-brand-light" : "border-chat-border bg-chat-surface text-slate-400 hover:border-slate-500"
    )}
  ><span>{emoji}</span><span>{count}</span></button>)}</div>}</div></div>;
}
