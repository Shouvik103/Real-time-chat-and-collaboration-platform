...
export function Sidebar() {
  ...
  const { data: channelsData } = useQuery({
    ...
    staleTime: 3e4,
    queryFn: () => workspaceApi.getChannels(activeWorkspace.id).then((r) => r.data.data.channels)
  });
  useEffect(() => {
    if (channelsData) {
      ...
      if (channelsData.length > 0) {
        ...
        const stillExists = channelsData.some((c) => c.id === currentActiveId);
  ...
  const createWsMutation = useMutation({
    mutationFn: (name) => workspaceApi.createWorkspace(name),
    onSuccess: (res) => {
  ...
  const createDmMutation = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.error?.message || "Failed to create DM")
  });
  const copyDmCode = () => {
    if (dmInviteCode) {
  ...
  const { data: membersData } = useQuery({
    ...
    queryFn: () => workspaceApi.getMembers(activeWorkspace.id).then((r) => r.data.data.members)
  });
  const inviteMutation = useMutation({
    mutationFn: (email) => workspaceApi.inviteMember(activeWorkspace.id, email),
    onSuccess: (res) => {
    ...
    onError: (err) => toast.error(err?.response?.data?.error?.message || "Failed to invite member")
  });
  ...
  const { data: inviteCodeData, refetch: refetchCode } = useQuery({
    ...
    queryFn: () => workspaceApi.getInviteCode(activeWorkspace.id).then((r) => r.data.data.inviteCode)
  });
  ...
  const joinMutation = useMutation({
    mutationFn: (code) => workspaceApi.joinByCode(code),
    onSuccess: (res) => {
      const ws = res.data.data.workspace;
      queryClient.invalidateQueries({ queryKey: ["workspaces"] });
      setActiveWorkspace(ws);
    ...
    onError: (err) => toast.error(err?.response?.data?.error?.message || "Invalid invite code")
  });
  const handleJoin = (e) => {
    e.preventDefault();
  ...
  ><ArrowRightEndOnRectangleIcon className="h-4 w-4" />
    ...
    title="Create Group"
  ><form onSubmit={handleCreateWorkspace} className="space-y-4"><Input
    label="Group Name"
    placeholder="e.g. Engineering Team"
    value={wsName}
    onChange={(e) => setWsName(e.target.value)}
    required
  /><div className="flex justify-end gap-2"><Button variant="ghost" type="button" onClick={() => setCreateWorkspaceModalOpen(false)}>
              Cancel
            </Button><Button type="submit" loading={createWsMutation.isPending}>
              Create
  ...
  >{!dmInviteCode ? <div className="space-y-4"><p className="text-sm text-slate-400">
              Create a private 1-on-1 chat. You'll get an invite code to share with the other person.
            </p><div className="flex justify-end gap-2"><Button variant="ghost" type="button" onClick={() => setCreateDmModalOpen(false)}>
                Cancel
              </Button><Button
    type="button"
  ...
  >
                Create DM
              </Button></div></div> : <div className="space-y-4"><p className="text-sm text-slate-400">
              Share this code with the person you want to chat with. They can use it to join the conversation.
            </p><div className="flex items-center gap-2"><code className="flex-1 rounded-md bg-chat-surface border border-chat-border px-3 py-2.5 text-sm text-white font-mono select-all break-all">{dmInviteCode}</code><Button size="sm" type="button" onClick={copyDmCode}><ClipboardDocumentIcon className="h-4 w-4" /></Button></div><div className="flex justify-end"><Button variant="ghost" type="button" onClick={() => setCreateDmModalOpen(false)}>
                Done
  ...
  ><form onSubmit={handleInvite} className="space-y-4"><Input
    label="Email address"
    type="email"
  ...
  /><p className="text-xs text-slate-400">
          ...
          </p><div className="flex justify-end gap-2"><Button variant="ghost" type="button" onClick={() => setInviteMemberModalOpen(false)}>
              Cancel
            </Button><Button type="submit" loading={inviteMutation.isPending}>
              Invite
            </Button></div></form></Modal>{
    /* Members list modal */
  }<Modal
  ...
  ><div className="space-y-2 max-h-80 overflow-y-auto">{membersData ? membersData.map((m) => <div key={m.id} className="flex items-center gap-3 py-1.5"><div className="h-8 w-8 rounded-full bg-brand/30 flex items-center justify-center text-sm font-bold text-brand-light shrink-0">{m.displayName[0].toUpperCase()}</div><div className="min-w-0 flex-1"><p className="text-sm text-white truncate">{m.displayName}</p><p className="text-xs text-slate-400 truncate">{m.email}</p></div><span className="text-xs text-slate-500 capitalize shrink-0">{m.role.toLowerCase()}</span></div>) : <p className="text-sm text-slate-400 text-center py-4">Loading…</p>}</div><div className="flex justify-end mt-4 pt-3 border-t border-chat-border"><Button variant="ghost" type="button" onClick={() => setMembersOpen(false)}>
            Close
          </Button></div></Modal>{
  ...
  ><p className="text-sm text-slate-400 mb-3">
        ...
        </p><div className="flex items-center gap-2"><code className="flex-1 rounded-md bg-chat-surface border border-chat-border px-3 py-2 text-sm text-white font-mono select-all break-all">{inviteCodeData ?? "Loading\u2026"}</code><Button size="sm" type="button" onClick={copyInviteCode} disabled={!inviteCodeData}>
            Copy
          </Button></div><div className="flex justify-between items-center mt-4 pt-3 border-t border-chat-border"><Button
    variant="ghost"
    size="sm"
    type="button"
  ...
  >
            Regenerate
          </Button><Button variant="ghost" type="button" onClick={() => setInviteCodeOpen(false)}>
            Close
          </Button></div></Modal>{
    /* Join by code modal */
  }<Modal
    ...
    title="Join by Code"
  ><form onSubmit={handleJoin} className="space-y-4"><Input
    label="Invite Code"
  ...
  /><p className="text-xs text-slate-400">
          ...
          </p><div className="flex justify-end gap-2"><Button variant="ghost" type="button" onClick={() => setJoinByCodeModalOpen(false)}>
              Cancel
            </Button><Button type="submit" loading={joinMutation.isPending}>
              Join
            </Button></div></form></Modal>{
  ...
  ><p className="text-sm text-slate-300 mb-1">
          Are you sure you want to delete{" "}<span className="font-semibold text-white">{deleteTargetWs?.name}</span>?
        </p><p className="text-xs text-slate-400 mb-5">
          This will permanently delete all messages. This cannot be undone.
        </p><div className="flex justify-end gap-2"><Button variant="ghost" type="button" onClick={() => setDeleteWsConfirmOpen(false)}>
            Cancel
          </Button><Button
    type="button"
