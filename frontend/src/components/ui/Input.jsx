...
export const Input = forwardRef(
  ({ label, error, leftAddon, className, id, ...props }, ref) => {
    ...
    return <div className="flex flex-col gap-1">{label && <label
      htmlFor={inputId}
      className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-400"
    >{label}</label>}<div className="relative flex items-center">{leftAddon && <span className="absolute left-3 text-slate-400/80">{leftAddon}</span>}<input
      ref={ref}
      id={inputId}
      className={clsx(
      ...
      {...props}
    /></div>{error && <p className="text-xs text-red-400 mt-0.5">{error}</p>}</div>;
  }
);
